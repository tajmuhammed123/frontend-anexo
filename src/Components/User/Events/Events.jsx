...
function Events() {
  ...
  useEffect(() => {
    queryClient.prefetchQuery(["eventlistdata", active], () =>
      fetchData(active)
    );
  }, [active, queryClient]);

  const { isLoading } = useQuery(["eventlistdata", active], fetchData);
  ...
  const renderPageButtons = () => {
    ...
    return pages.map((page) => (
      <Button
        ...
        onClick={() => {
          setActive(page);
          queryClient.invalidateQueries("eventlistdata");
        }}
      >
        {page}
      </Button>
  ...
  //     const nameMatch = user.eventData.team_name
  //       .toLowerCase()
  ...
  //   });
    ...
    const handleSearch=async()=>{
      try {
        await axiosUserInstance.get(
          `/eventsearch?search=${searchInput}&name=${name}`
        ).then((res)=>setData(res.data.managers))
      } catch (error) {
          ...
          <div className="flex flex-wrap justify-center pt-10">
            {data ? (
              data.map((card, index) => {
                ...
                return (
                  <Card
                    className="w-full max-w-[17rem] h-[30rem] shadow-lg mx-5"
                    key={index}
