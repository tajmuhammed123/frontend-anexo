      ...
      const { isLoading, error } = useQuery({
        ...
        queryFn: async () => {
          try {
            ...
            const response = await axiosManagerInstance.get(`/bookeddetails/${id}`,config);
      ...
      const handlePermission=async()=>{
        try {
          ...
          const managerInfo=JSON.parse(managerData)
            ...
            const res=await axiosManagerInstance.get(`/confirmbooking/${amount}/${id}`,config)
            if(res.data.status){
    ...
    return (
      ...
      <Card color="transparent" shadow={false} className="w-full max-w-[40rem]">
        ...
        <CardBody className="mb-6 p-0">
          ...
          {data.is_paid=='not paid'?<div className="flex justify-around my-6">
            <Button color="green" onClick={handleConfirmOpen}>Confirm</Button>
      ...
      <Dialog open={confirmOpen} size="xs" handler={handleConfirmOpen}>
        ...
        <DialogBody>
          <div className="grid gap-6">
            ...
            <Input label="Amount" type="number" onChange={(e)=>setAmount(e.target.value)} />
          </div>
        </DialogBody>
